...
class BenchBase(benchName: String)
    extends TestKit(ActorSystem("benchName"))
    with ImplicitSender
  ...
  def benchTest(
    ...
    val subs = system.actorOf(
      Props(classOf[SubsActor], testActor, topic, qos, brokerUrl)
    )
    ...
    def askLoop(runCount: Int): Future[Done] = {
      if (runCount >= maxRunCount) {
        ...
        println(
          s"$benchName/$qos/$runCount: received $receivedCount = ${receivedCount * 100.0 / msgCount}%"
        )
    ...
    println(
      s"$benchName/$qos done in ${(System.currentTimeMillis() - timeStart).toDouble / 1000} seconds"
    )
  }
...
private class PubActor(topic: String, qos: Int, brokerUrl: String, count: Int)
    extends Actor {
  private val pubsub = {
    val cfg = PSConfig(
      brokerUrl,
...
private class SubsActor(
  ...
  private val pubsub =
    context.actorOf(Props(classOf[MqttPubSub], PSConfig(brokerUrl)))
